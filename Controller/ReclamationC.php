<?php
	...
	require_once 'C:/xampp/htdocs/test/Model/Reclamation.php';
	...
	class ReclamationC {
		...
		function searchrec($s){
			$sql='SELECT * FROM recl WHERE (prenom_reclamation LIKE "%'.$s.'%") OR (nom_reclamation LIKE "%'.$s.'%") ';
			$db = connexiondb::getConnexion();
			try{
				$liste = $db->query($sql);
				return $liste;
